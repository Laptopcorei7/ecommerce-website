    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{products.length} products in store</p>
        </div>
                    ...
                    <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <img
                            src={product.images?.[0] || product.image || `https://placehold.co/40x40/e2e8f0/64748b?text=P`}
                            alt={product.name}
                            className="w-10 h-10 object-cover rounded-lg bg-gray-100 shrink-0"
                            onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/40x40/e2e8f0/64748b?text=P'; }}
                          />
                          <div>
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          <Input label="Product Name" placeholder="e.g. Wireless Headphones" required {...field('name')} />
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Price ($)" type="number" placeholder="29.99" min="0" step="0.01" required {...field('price')} />
            <Input label="Compare Price ($)" type="number" placeholder="39.99" min="0" step="0.01" {...field('comparePrice')} hint="Original price for discount display" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input label="Category" placeholder="Electronics" required {...field('category')} />
            <Input label="Stock Quantity" type="number" placeholder="100" min="0" required {...field('stock')} />
          </div>
          <Input
            label="Image URLs"
