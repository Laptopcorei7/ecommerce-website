...
export function OrderStatusBadge({ status }: { status: OrderStatus }) {
  const { label, classes } = statusConfig[status] ?? { label: status, classes: 'bg-gray-100 text-gray-700' };
  return (
...
export default function Badge({ children, color = 'gray' }: BadgeProps) {
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${colorMap[color]}`}>
      {children}
    </span>
  );
}
